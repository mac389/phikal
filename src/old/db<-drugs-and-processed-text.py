...
READ = 'rb'
WRITE = 'wb'
stopwords = set(open('../data/stopwords',READ)) | set(nltk.corpus.stopwords.words('english'))
punctuation = string.punctuation + "`'"
contraction_expansion = json.load(open('../data/contraction-expansion.json',READ))
lmtzr = WordNetLemmatizer()
db = json.load(open('../data/db.json',READ))
drugs = json.load(open('../data/drug_misnaming.json',READ))
not_drugs = set(open('../data/not_drug_names',READ).read().splitlines())

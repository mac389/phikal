...
drugs = open('../data/effects-that-actually-occurred',READ).read().splitlines()
m = np.loadtxt('../data/effect-matrix-actually-occurred.tsv',delimiter='\t').astype(int)
m += 1
m /= 2
...
plt.tight_layout()
plt.savefig('../imgs/overall-effect-frequency.png')
